...
```
结合 GitHub Actions 官方文档和 `gh` CLI，在一个新建的公共实验仓库中设计、提交并运行一组 Workflows（仅用于实验与学习，不连接生产环境、不修改真实系统），覆盖以下场景：
1. 有先后依赖关系的 jobs。
2. 只在特定条件下触发的 workflow。
3. 会按固定时间自动运行的 workflow。
4. 在 pull request 上运行、且失败时会阻塞合并的检查。
5. 可能偶发失败、需要自动重试的步骤。
6. 能接收外部输入参数、且不同输入下行为不同的 workflow。
7. 依赖仓库外部配置或运行时注入信息的 workflow。
8. 支持手动重跑，并比较不同重跑方式差异的 workflow。
9. 可能超时，并能观察超时、中断与普通失败差异的 workflow。
要求：
- 实时监控运行状态；
- 导出并阅读日志；
- 识别关键失败信号；
- 通过修改配置、输入或策略，使各类行为符合预期；
- 最后沉淀为一份关于 GitHub Actions 实战编写与调试的 skill。
```
...
agent 产出的代码、写的文档、提交的 PR 描述，本身就是知识的载体。组织良好的代码仓库，agent 读代码就能学到项目惯例；详尽的 PR 描述，下次做类似功能时就是最好的参考。

这一层不需要额外维护，但需要**有意识地让产出本身承载知识**——要求 PR 描述写清楚"为什么这么改"而不只是"改了什么"，在反直觉的地方留注释。

### 独立知识库的内部结构：两个维度

三种载体中，AGENTS.md 足够简单不需要额外结构，代码和文档的组织由项目本身决定。真正需要设计的是独立知识库内部怎么组织。用两个正交的维度来切分：**横切按类型，竖切按受众**。
